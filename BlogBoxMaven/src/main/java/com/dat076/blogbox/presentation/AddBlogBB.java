    ...
    public String createBlog(BlogBoxUser user) {
        ...
        for (Blog temp : blogs) {
            if (temp.getName().equals(blog.getName()) || blog.getName().equals("")) {
                ...
                return "returnToUser";//"user?faces-redirect=true&includeViewParams=true";
            }
        }
        blogFacade.create(blog);
        blogBoxUserView.setBlog(blog);
        return "enterBlog";//"blog?faces-redirect=true&includeViewParams=true";
    }
